    ...
    geometries: n === 1 ? [{type: "Sphere"}]
      ...
      : points.map(function(_, i) {
        ...
        do {
          ...
          if (dot(centre, centre0) < oneE_minus_15 - 1) {
...
function circumcentre(t) {
  var p0 = t.a.p,
      ...
      a = .5 * m2 * norm2(subtract(p1, p2)) * dot(subtract(p0, p1), subtract(p0, p2)),
      b = .5 * m2 * norm2(subtract(p0, p2)) * dot(subtract(p1, p0), subtract(p1, p2)),
      c = .5 * m2 * norm2(subtract(p0, p1)) * dot(subtract(p2, p0), subtract(p2, p1)),
      centre = [
      ...
      k = norm2(centre);
  if (k > oneE_minus_15) centre[0] *= (k = 1 / Math.sqrt(k)), centre[1] *= k, centre[2] *= k;
  else centre = t.n;
  if (!visible(t, centre)) centre[0] *= -1, centre[1] *= -1, centre[2] *= -1, radius = pie - radius, centre.negative = true;
...
function visible(t, p) {
  return dot(t.n, p) - dot(t.n, t.a.p) > oneE_minus_15;
}

function coplanar(t, p) {
  return Math.abs(dot(t.n, p) - dot(t.n, t.a.p)) <= oneE_minus_15;
}
...
function cartesian(spherical) {
  var a = spherical[0] * radians,
      b = spherical[1] * radians,
      cosb = Math.cos(b);
  return [
    cosb * Math.cos(a),
    cosb * Math.sin(a),
